...
@router.get("/air-quality-fetch/{neighborhood_id}")
def air_quality_fetch(neighborhood_id: int, db: Session = Depends(get_db)) -> dict:
    """OpenAQ'dan canlı ölçüm çekip environmental_data'ya kaydeder."""
    neighborhood = db.get(Neighborhood, neighborhood_id)
    if not neighborhood:
        return {"status": "error", "message": "Neighborhood not found."}
    return fetch_and_persist_air_quality(neighborhood, db=db)
...
def green_area_analysis(neighborhood_id: int, db: Session = Depends(get_db)) -> dict:
    ...
    if not neighborhood:
        return {"status": "error", "message": "Neighborhood not found."}
    return analyze_neighborhood_green_area(neighborhood, db=db)
